...
import { useGetAllBlogCategories, useBlogCategories } from '@/hooks';
...
export const BlogCategoryPage: React.FC = () => {
  const { data: categories = [], isLoading } = useGetAllBlogCategories();
  const { deleteBlogCategory, isDeleting } = useBlogCategories();
  ...
  return (
    <div>
      <PageHeader
        title="مدیریت دسته‌بندی‌های مقالات"
        description="دسته‌بندی‌های مقالات را مشاهده و مدیریت کنید"
        breadcrumbItems={[
          {
            title: (
