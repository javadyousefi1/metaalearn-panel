...
import { useGetBlogGallery, useBlogGallery } from '@/hooks';
import { BlogGalleryUploadModal } from './BlogGalleryUploadModal';
import {
  BlogGalleryItem,
  GalleryType,
...
export const BlogGalleryPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [modalOpen, setModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<BlogGalleryItem | null>(null);
  const [failedImages, setFailedImages] = useState<Set<string>>(new Set());

  const { data: galleryData, refetch, isLoading } = useGetBlogGallery(
    {
      BlogId: id || '',
      PageIndex: 1,
      PageSize: 99999,
  ...
  const { deleteGallery, isDeleting } = useBlogGallery();
  ...
  const handleEditItem = (item: BlogGalleryItem) => {
    setEditingItem(item);
    setModalOpen(true);
  };
  ...
  const renderGalleryItem = (item: BlogGalleryItem) => (
    <div
      key={item.id}
            ...
            <Empty
              description={
                <div>
                  ...
                  <p className="text-gray-500">شناسه مقاله: {id}</p>
